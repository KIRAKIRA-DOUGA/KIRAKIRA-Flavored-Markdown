...
(function () {
    ...
    function parse(html) {
        const lines = (html + "\n").replaceAll(/<(?!!--)/g, "&gt;").split("\n");
        let result = "";
        let bold = false, italic = false, underline = false, highlight = false, superscript = false, subscript = false, strikethrough = false, inlineCode = false, quoteLayer = 0;
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            let line = lines[lineIndex];
            let lineResult = "", lineStack = "";
            // 缩进
            line = line.replace(/^\s+/, indentSpace => {
                countChar(indentSpace, " ");
                countChar(indentSpace, "\t");
                return "";
            }).trim();
            // 分割线
